...
def run_model(model_inputs, default_inputs):
    """
    import and run xgb model
    """
    ...
    # -----------------------------
    FEATURE_COLUMNS = ["YEAR","age_months_continuous","SPECTYPEV","TRI_SOURCES5k",
                       "NEI_SOURCES5k","med_income_pct_of_median","PCT_FAMILYPOVERTY",
                       "proportion_black","home_value_pct_of_median","YEAR_BUILT_2","PIPED_WATER1"]

    # test_input = {"YEAR": 2010, "age_months_continuous": 15.96, "SPECTYPEV": 0, "TRI_SOURCES5k": 1, "NEI_SOURCES5k": 2, 
    #               "med_income_pct_of_median": 0.559849, "PCT_FAMILYPOVERTY": 5.357143, "proportion_black": 0.2367434,
    #               "home_value_pct_of_median": 0.2379016, "YEAR_BUILT_2": 1957, "PIPED_WATER1": 0}

    df = pd.DataFrame([model_inputs])[FEATURE_COLUMNS]

    # Convert to DMatrix
    dmatrix = xgb.DMatrix(df)

    # Predict
    risk_probability = 1-model.predict(dmatrix)

    # -----------------------------
    # run for county/CBG for comparison
    # -----------------------------
    
    ddf = pd.DataFrame([default_inputs])[FEATURE_COLUMNS]

    # Convert to DMatrix
    ddmatrix = xgb.DMatrix(ddf)

    # Predict
    default_probability = 1-model.predict(ddmatrix)
...
# old code that imported and ran RF model
# import json
# import os
# import numpy as np
